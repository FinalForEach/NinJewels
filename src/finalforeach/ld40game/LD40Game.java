...
public class LD40Game extends ApplicationAdapter
{
  ...
  public void render()
  {
    ...
    if (!this.started)
    {
      ...
      ry = camera.position.y - 150;
      startrect = new Rectangle(rx + 4, ry - 100 - camera.viewportHeight / 4 + 255, 
        390, 41);
    ...
    Rectangle startrect = (mouseVec = mines).length;
    for (float ry = 0; ry < startrect; ry++)
    {
      Mine m = mouseVec[ry];
    ...
    batch.begin();
    drawBackground();
    startrect = (mouseVec = mines).length;
    for (ry = 0; ry < startrect; ry++)
    {
      Mine m = mouseVec[ry];
